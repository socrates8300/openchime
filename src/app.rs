...
use iced::{Application, Command, Element, Theme, Length};
...
impl Application for OpenChimeApp {
    ...
    fn new((db, audio): Self::Flags) -> (Self, Command<Message>) {
        ...
        let startup_command = Command::perform(async move {
            ...
            .await {
                Ok(events) => {
                    log::info!("Loaded {} events on startup", events.len());
                    for event in &events {
                        log::debug!("Event: {} at {}", event.title, event.start_time.with_timezone(&chrono::Local));
                    }
                    events
                },
                Err(e) => {
...
impl Drop for OpenChimeApp {
    fn drop(&mut self) {
        info!("========================================");
        info!("OpenChimeApp shutdown sequence initiated");
        info!("========================================");
        ...
        // Note: Database connection pool (self.db) is Arc'd and will be closed 
        // automatically when the last reference is dropped. We don't need to 
        // explicitly block on close() here, which avoids runtime panics.

        info!("OpenChimeApp shutdown complete");
        info!("========================================");
    }
}
